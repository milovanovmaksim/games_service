...
from games.models import EsrbRating, Player


def create_esrb_rating(client, description):
    url = reverse(views.EsrbRatingList.name)
    esrb_rating_data = {'description': description}
    esrb_rating_responce = client.post(url, esrb_rating_data, format='json')
    return esrb_rating_responce
...
def test_create_duplicated_esrb_rating(client):
    ...
    post_response1 = create_esrb_rating(client, new_esrb_rating_description)
    assert post_response1.status_code == status.HTTP_201_CREATED
    post_response2 = create_esrb_rating(client, new_esrb_rating_description)
...
@pytest.mark.django_db
def test_create_and_retrieve_palyer(client):
...
def test_create_duplicate_player(client):
    new_palyer_name = 'Fergie'
    new_player_gender = Player.FEMALE
    post_response1 = create_player(client, new_palyer_name, new_player_gender)
    assert post_response1.status_code == status.HTTP_201_CREATED
    post_response2 = create_player(client, new_palyer_name, new_player_gender)
    assert post_response2.status_code == status.HTTP_400_BAD_REQUEST


@pytest.mark.django_db
def test_retrieve_players_list(client):
    new_palyer_name = 'Vanessa Perry'
    new_player_gender = Player.FEMALE
    create_player(client, new_palyer_name, new_player_gender)
    url = reverse('player-list')
    get_response = client.get(url, format='json')
    assert get_response.status_code == status.HTTP_200_OK
    assert get_response.data['count'] == 1
    assert get_response.data['results'][0]['name'] == new_palyer_name
    assert get_response.data['result'][0]['gendare'] == new_player_gender
